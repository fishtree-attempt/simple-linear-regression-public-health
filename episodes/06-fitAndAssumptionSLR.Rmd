...
BPSysAve_AgeMonths_lm <- dat %>% lm(formula = BPSysAve ~ AgeMonths)
```
...
```{r variation in outcome, echo = FALSE}
set.seed(22)

lowVar <- tibble(Weight = rnorm(50, mean = 80, sd = 5),
                 Height = rnorm(50, 170, 10)) 

for(i in 1:nrow(lowVar)){
  ifelse(lowVar$Height[i] > 170,
         lowVar$Weight[i] <- lowVar$Weight[i] + 5*lowVar$Height[i]/(mean(lowVar$Height)),
         lowVar$Weight[i] <- lowVar$Weight[i] - 5*lowVar$Height[i]/(mean(lowVar$Height))) 
}


highVar <- tibble(Weight = rnorm(50, mean = 80, sd = 10),
                 Height = rnorm(50, 170, 10))

for(i in 1:nrow(highVar)){
  ifelse(highVar$Height[i] > 170,
         highVar$Weight[i] <- highVar$Weight[i] + 5*highVar$Height[i]/(mean(highVar$Height)),
         highVar$Weight[i] <- highVar$Weight[i] - 5*highVar$Height[i]/(mean(highVar$Height))) 
}

lowVarMin <- min(lowVar$Weight)
lowVarMax <- max(lowVar$Weight)
highVarMin <- min(highVar$Weight)
highVarMax <- max(highVar$Weight)


p1 <- ggplot(lowVar, aes(x = Height, y = Weight)) +
  geom_point() +
  ylim(40, 115) +
  xlim(145, 195) +
  ggtitle('Lower variation in weight') +
  geom_segment(x = 193, xend = 193, y = lowVarMin, yend = lowVarMax, 
               arrow = arrow(length = unit(0.1, "inches"), ends = "both",
                             type = "closed"),
               linetype = 2, color = "red", size = 1.2)

p2 <- ggplot(highVar, aes(x = Height, y = Weight)) +
  geom_point() +
  ylim(40, 115) +
  xlim(145, 195) +
  ggtitle('Higher variation in weight') +
  geom_segment(x = 193, xend = 193, y = highVarMin, yend = highVarMax, 
               arrow = arrow(length = unit(0.1, "inches"), ends = "both",
                             type = "closed"),
               linetype = 2, color = "red", size = 1.2)
